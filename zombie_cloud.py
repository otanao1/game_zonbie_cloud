...
if __name__ == "__main__":
    ...
    def day_four(self):
        ...
        if choice == "A":
            ...
            self.slow_print("しかし、返答があるまでに数時間かかります...")
            self.slow_print("その間にもネットワークの分断は広がり、サービスは次々とダウンしていきます。")
            self.update_score(-15 + network_penalty)
            self.network_isolated = True
    ...
    def day_five(self):
        ...
        if choice == "A":
            ...
            if random.random() < 0.6:  # 60%の確率で成功
                self.slow_print("\nあなたはAmazon Qのアドバイスに従い、複数の対策を組み合わせました。")
                self.slow_print("WAFとShieldの組み合わせにより、攻撃トラフィックを効果的にブロック！")
                self.slow_print("サービスは徐々に回復しています。")
                self.update_score(15 + ddos_penalty)
                self.ddos_mitigated = True
            else:
                self.slow_print("\nAmazon Qのアドバイスに従おうとしましたが、")
                self.slow_print("設定の複雑さと攻撃の規模により、完全な対応ができませんでした。")
                self.slow_print("攻撃は部分的に緩和されましたが、サービスは依然として不安定です...")
                self.update_score(-10 + ddos_penalty)
                self.ddos_mitigated = False
